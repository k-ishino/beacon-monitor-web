...
```
ble_scan_usb/     ble_scan_usb.ino, ratelimit.h      元プロジェクトから無改造で移植（ファームウェアは変更しない）※移植済み
web/              index.html                         iBeacon 専用モニタ（今回の主な改修対象）※実装済み
                  setup.html                          書き込みページ（ESP Web Tools、元プロジェクトから移植）※未着手
tools/esp-web-tools/                                  esp-web-tools をバンドルする依存の固定（移植）※未着手
.github/workflows/build.yml                          ビルド → GitHub Pages へデプロイ ※未着手
```

元プロジェクトとの主な違いは配信先です。元は Cloudflare Workers でしたが、本リポジトリは
**GitHub Actions + GitHub Pages** で完結させます（Cloudflare アカウントを持たないため）。
ビルド内容（arduino-cli によるファームウェアビルド、esp-web-tools のバンドル）自体は
元プロジェクトと同じ仕組みを踏襲します。
...
`web/setup.html`（元プロジェクトから移植予定、変更なし）。ESP Web Tools を使い、Arduino IDE を
使わずブラウザだけでファームウェアを書き込みます。ファームウェア自体は無改造のため、
書き込み対象の `.bin` はビルドパイプラインで元プロジェクトと同じ手順により生成します。

## 配信（GitHub Actions + GitHub Pages、未着手）
...
Cloudflare を使わない点のみ元プロジェクトと異なり、ビルドの中身は同一です。
...
2. ~~`web/index.html` の iBeacon 専用改修~~ → **完了(本版)**
3. `web/setup.html`・`tools/esp-web-tools/` の移植
4. `.github/workflows/build.yml` を GitHub Pages 向けに調整
5. GitHub Pages 公開、公開先 URL をこの README 冒頭に記載
6. 通し動作確認(実機での UUID フィルタ・CSV ログ・コメント永続化の確認を含む)
